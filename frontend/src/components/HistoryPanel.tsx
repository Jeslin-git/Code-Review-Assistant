      ...
      {reviews.map((review) => (
        ...
        >
          <div className="flex justify-between items-center">
            <span className="text-xs font-semibold text-blue-400 uppercase">{review.mode}</span>
            <span className="text-xs text-gray-500">{new Date(review.createdAt).toLocaleDateString()}</span>
          </div>
          {selected?.id === review.id && (
            <div className="mt-2 text-xs text-gray-400">
              <p className="mb-2">{review.result?.summary}</p>
              {review.result?.issues?.slice(0, 3).map((issue, i) => (
                <p key={i} className="text-gray-500">• {issue.title}</p>
              ))}
            </div>
          )}
        </div>
